...
# Modified for third-person view
...
## Height offset of camera from player
...
func update_camera_position(delta: float = 0.0) -> void:
	...
	if delta > 0:
		camera.global_position = camera.global_position.lerp(target_pos, delta * camera_smoothness)
	else:
		camera.global_position = target_pos

func rotate_camera(rot_input : Vector2):
	camera_rotation.x -= rot_input.y * look_speed
	camera_rotation.x = clamp(camera_rotation.x, deg_to_rad(-30), deg_to_rad(60))  # Limit vertical rotation
	camera_rotation.y -= rot_input.x * look_speed
